							...
							<table class="table-list-search">
								...
								<tbody>
									...
									<tr>
										<th>프로젝트 기간</th>
										<td colspan="3">
											<input type="text" id="search-st-date" class="common-input w10 datepicker"
												onchange="getList(1)" />
											<span class="inblock tc" style="width: 20px;">-</span>
											<input type="text" id="search-ed-date" class="common-input w10 datepicker"
												onchange="getList(1)" />
										</td>
									</tr>
									<!-- 검색버튼, 초기화버튼 영역 -->
...
?>
						html += '	</td>';
						html += '	<td>';
						html += '		<a href="<?= $livesympoUrl ?>/' + item.PRJ_TITLE_URI + '" target="_stream">' + item
							.PRJ_TITLE_URI + '</a>';
						html += '	</td>';
					...
					let btnHtml = '';
					pages.forEach(item => {
						btnHtml += '<button class="page-button ' + (pageNo == item ? 'active-page' : '') +
							'" onclick="changePage(' + item + ')" ' + (pageNo == item ? 'disabled' : '') + '>' + item +
							'</button>';
					});
					if (btnHtml === '') {
